...
from irlmethods import irlUtils  # NOQA: E402
from neural_nets.base_network import BaseNN  # NOQA: E402
...
class DeepMaxEnt():
    ...
    def expert_svf(self):
        """
        Calculates expert from irl utilities and returns it.
        """
        return irlUtils.expert_svf(
            self.traj_path,
            state_dict=self.rl.feature_extractor.state_dictionary
        )
    ...
    def per_state_reward(self, reward_function):
        ...
        all_states = torch.tensor(
            all_state_list, dtype=torch.float).to(self.device)
    ...
    def plot(self, images, titles, save_path=None):
        ...
        for image_idx, image in enumerate(images):
            ...
            colorbars.append(plt.colorbar(image))
    ...
    def train(self):
        ...
        for i in range(self.max_episodes):
            ...
            reward_network_folder = './saved-models-rewards/' + \
                'reg-'+str(self.regularizer)+'/'

            pathlib.Path(reward_network_folder).mkdir(
            ...
            policy_network_folder = './saved-models/' + \
                'reg-'+str(self.regularizer)+'/'
            pathlib.Path(policy_network_folder).mkdir(
                parents=True, exist_ok=True)
            current_agent_policy.save(policy_network_folder)

            diff_freq = - \
                torch.from_numpy(expertdemo_svf -
                                 current_agent_svf).type(self.dtype)
