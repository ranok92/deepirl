...
def getStateVisitationFreq(policyfile , rows = 10 , cols = 10 , num_actions = 5):
	...
	policy.load_state_dict(torch.load(policyfile))
	policy.eval()
	policy.to(DEVICE)

	stateActionTable = createStateActionTable(policy, rows, cols, env.action_space.n)
	stateTransitionMatrix = createStateTransitionMatix(rows= rows,cols = cols, action_space = env.action_space.n)
