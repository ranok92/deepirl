...
from featureExtractor.gridworld_featureExtractor import FrontBackSide
...
def main():
    ...
    featExtract = FrontBackSide(fieldList = ['agent_state','goal_state','obstacles'])

    env = GridWorld(display=args.render, is_onehot= False, obstacles=[np.asarray([1, 2])])

    model = ActorCritic(env, feat_extractor=featExtract,  gamma=0.99, log_interval=100, max_episodes=10**4,
                        max_ep_length=30)
